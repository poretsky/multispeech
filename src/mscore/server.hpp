...
//
// The first two arguments of the constructor are usually passed
// from main(). The last argument specifies a command that should be
// generated by the request() method to finish session gracefully
// when the source stream is exhausted.
//
...
protected:
  // Construct / destroy:
  server(int argc, char* argv[], const char* session_break_command);
  virtual ~server(void);
